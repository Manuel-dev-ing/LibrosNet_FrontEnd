...
import { editEditorial } from '../../../services/EditorialAPI';
...
export default function EditEditorialModal() {
  const dataEditorial = useLibrosStore( (state) => state.data as EditorialFormData )
  ...
  useEffect( () => {
    ...
    if (activeId) {
      reset({
        nombre: dataEditorial.nombre,
        correo: dataEditorial.correo,
        telefono: dataEditorial.telefono,
        calle: dataEditorial.calle,
        numero: dataEditorial.numero,
        colonia: dataEditorial.colonia,
        ciudad: dataEditorial.ciudad,
        codigoPostal: dataEditorial.codigoPostal,
        estado: dataEditorial.estado
      })

    }

  }, [dataEditorial, activeId]) 

  const queryClient = useQueryClient()

  const mutation = useMutation({
    mutationFn: editEditorial,
    onError: (error) => {
  ...
  return (
    <>
      <div className="modal fade" id="editarmodalEditorial" aria-labelledby="exampleModalLabel" aria-hidden="true" ref={modalRef}>
